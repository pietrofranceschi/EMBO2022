...
* extrapolate the retention time on regions of the rt axes where the peaks are not present (either with a linear or aloess function)
...
Let's dig more on this. For example, can we visualize the position of the peaks assigned to FT005. The idx are the indexes of the row in the peak tabls
...
NAs will be discussed in more details by Maria in the afternoon, here we can anticipate that the two previous processess will result in different type of missingness (either random or non random) ...
...
* At the beginning of this demo we have been focussing our attention to the extracted ion trace around mz 289. Try to check how many features were defined there. Can you check i fthe oeak was integrated well?
