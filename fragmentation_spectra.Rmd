...
Fragmentation experiments, often called MS/MS, are an essential tool for this task. They can be used either to deduce information on the chemical characteristics of unknown compounds, or to confirm the annotation when autentic chemical standards are available.
...
Let's now make anicer plot with the spectra
...
points(mz(focus_MS_MS[[2]]), intensity(focus_MS_MS[[2]])/max(intensity(focus_MS_MS[[2]])), col = "red")
points(mz(focus_MS_MS[[4]]), intensity(focus_MS_MS[[4]])/max(intensity(focus_MS_MS[[4]])), col = "blue")
```
...
```{r}
wine <- readMSData("data/x004_red_sangio_B_1_NEG_DDA.mzML", 
                   mode = "onDisk")
```
...
wine %>% 
  filterMsLevel(1) %>% 
  filterRt(c(200,300)) %>% 
  chromatogram(mz = 577.13 + c(-1,1)*0.01) %>% 
  plot()
```
...
```{r}
plot(mz(MSMS[[1]]), 
     intensity(MSMS[[1]])/max(intensity(MSMS[[1]])), 
     type = "h",
     xlab = "mz",
     ylab = "normalized Intensity")
```
